from CRUD import from_collection_to_df, provide_db_connection
...
def manipulate_realtime_info(path_geojson, db_name, collection, city=False):
    ...
    new_countries = pd.DataFrame(
        [name["properties"]["ADMIN"] for name in countries["features"]],
         columns=["country"])
    df_country.replace({"country" : {"Korea, South" : "South Korea",
    ...
    fig = px.choropleth_mapbox(df_new[['country', 'confirmed', 'log_confirmed']], geojson=countries, locations='country', featureidkey="properties.ADMIN",
                           color="log_confirmed",
                           hover_name="country",
                           hover_data=["confirmed"],
                           color_continuous_scale="Reds",
                           mapbox_style="carto-positron", zoom=0, labels={"log_confirmed": "Count"})
